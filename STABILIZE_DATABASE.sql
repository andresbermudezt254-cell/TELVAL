-- =========================================================================
-- TELVAL - SCRIPT COMPLETO DE ESTABILIZACIÓN DE BASE DE DATOS (RLS + ENUMS + RPC)
-- Copia y pega este contenido en el SQL Editor de Supabase y ejecútalo (Run).
-- =========================================================================

-- 1. ASEGURAR VALORES DE ENUMS
DO $$
BEGIN
...
CREATE POLICY "usuarios_read_policy" ON public.usuarios
  ...
  USING (id = auth.uid() OR public.current_user_role_safe() IN ('admin', 'superadmin', 'almacen'));
...
-- 4. POLÍTICAS RLS PARA requisiciones
...
CREATE POLICY "requisiciones_select_policy" ON public.requisiciones
  FOR SELECT TO authenticated
  USING (empleado_id = auth.uid() OR public.current_user_role_safe() IN ('admin', 'almacen', 'superadmin'));
...
CREATE POLICY "requisiciones_insert_policy" ON public.requisiciones
  ...
  WITH CHECK (empleado_id = auth.uid() OR public.current_user_role_safe() IN ('admin', 'superadmin'));
...
CREATE POLICY "requisiciones_update_policy" ON public.requisiciones
  ...
  USING (empleado_id = auth.uid() OR public.current_user_role_safe() IN ('admin', 'almacen', 'superadmin'))
  WITH CHECK (empleado_id = auth.uid() OR public.current_user_role_safe() IN ('admin', 'almacen', 'superadmin'));
...
CREATE POLICY "requisiciones_delete_policy" ON public.requisiciones
  ...
  USING ((empleado_id = auth.uid() AND estado = 'BORRADOR') OR public.current_user_role_safe() IN ('admin', 'superadmin'));

-- 5. POLÍTICAS RLS PARA detalle_requisicion
  ...
  USING (
    EXISTS (
      ...
      WHERE r.id = requisicion_id
        AND (r.empleado_id = auth.uid() OR public.current_user_role_safe() IN ('admin', 'almacen', 'superadmin'))
    )
  );
...
CREATE POLICY "detalle_insert_policy" ON public.detalle_requisicion
  ...
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.requisiciones r
      WHERE r.id = requisicion_id
        AND (r.empleado_id = auth.uid() OR public.current_user_role_safe() IN ('admin', 'superadmin'))
    )
  );
...
-- 6. POLÍTICAS RLS PARA notificaciones
...
-- 7. PROCEDIMIENTO ALMACENADO ATÓMICO: marcar_item_recibido_v2
