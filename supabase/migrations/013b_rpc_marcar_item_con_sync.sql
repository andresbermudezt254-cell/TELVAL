-- Migration: 013b_rpc_marcar_item_con_sync
-- Purpose: Create a stored procedure that syncs the user AND marks the item complete
...
BEGIN
  ...
  IF v_count = 0 THEN
    RETURN jsonb_build_object('error', 'Item no encontrado o no actualizado');
  END IF;
