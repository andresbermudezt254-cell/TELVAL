import { Plus, Check, Tag, TrendingDown, Building2, AlertCircle } from 'lucide-react'
...
export function ProductCard({ product }: ProductCardProps) {
  ...
  const isInCart = items.some((i) => i.producto.id === product.id && i.producto.proveedor_id === product.proveedor_id)
    ...
    <div className="bg-white rounded-2xl border border-slate-200/80 hover:border-blue-300 hover:shadow-xl hover:-translate-y-1 transition-all duration-200 flex flex-col group overflow-hidden shadow-xs">
      ...
      <div className="flex flex-col flex-1 p-3.5 gap-2">
            ...
            <div className="space-y-2">
              ...
              <div>
                <div className="flex items-baseline gap-1">
                  <p className="text-lg font-bold text-[#1e3a5f]">{formatCOP(product.precio_unitario)}</p>
        ...
        <button
          ...
          disabled={!product.proveedor_id || product.total_proveedores === 0}  // ✅ Deshabilitar si no hay proveedor
          className={`flex items-center justify-center gap-1.5 w-full py-2 rounded-xl text-sm font-semibold transition-all duration-150 ${
            !product.proveedor_id || product.total_proveedores === 0
              ? 'bg-gray-100 text-gray-400 cursor-not-allowed'  // ✅ Sin proveedor
              : isInCart
        ...
        >
          ...
          {!product.proveedor_id || product.total_proveedores === 0 ? 'Sin proveedor' : isInCart ? 'Agregado' : 'Agregar al carrito'}
        </button>
      </div>
    </div>
  )
